	...
	public String bookSeat(@ModelAttribute("Seat") Seat seat, @RequestParam("movieName") String movieName,
		...
		if (object == null) {
			...
			if (((date.isAfter(currentDate)) || (date.equals(currentDate)))
				...
				System.out.println("This date has been booked already");
			...
			if (((date.isAfter(currentDate)) || (date.equals(currentDate)))
				 ...
				 Date date2 = Date.from(date.atStartOfDay(defaultZoneId).toInstant());
				    ...
				    m.addAttribute("seats", seatNo1);
				    session.setAttribute("user", object);
				    session.setAttribute("msg", "your seat book successfully");

				    // Send email 
				    MimeMessage message = emailSender.createMimeMessage();
				    MimeMessageHelper helper = new MimeMessageHelper(message, "utf-8");
				    String recipientName = object.getName();
				    String emailBody = "<html><body><p style='color: black; font-weight: bold;'>Dear " + recipientName + ",</p>\n\n" +
				            "<p>Your ticket have been booked successfully with Kite Cinemas " +
				            
				            "<p style='color: black; font-weight: bold;'>Booking Details:</p>\n\n" +
				            
				            "<ul style='color: brown; font-weight: bold;'>" +
				            "<table border='1' width='300px' style=text-align:center;font-size:20px;'><tr><td colspan='2'>"+
				            "</td></tr><tr><td>Movie Name</td><td>"+ movieName +"</td></tr><tr><td>Booking Date</td><td>"+
				            date.toString() + "</td></tr><tr><td>Booking time</td><td>"+time +"</td></tr><tr><td>Number of Seats</td><td>"+seat.getSeatNo().size() +"</td></tr><tr><td>Seat Number</td><td>"+seat.getSeatNo() +"</td></tr><tr><td>Amount ₹  </td><td>"
				            + sum +"</td></tr></table>"+
				           
				            
				            "<p style='color: black; font-weight: bold;'>We hope you enjoy the movie! If you have any further questions or concerns, feel free to contact us.</p>\n\n" +
				           
				            "<p style='font-style: italic; color: black; font-weight: bold;'>Thanks,</p>\n\n" +
				           
				            "<p style='font-weight: bold;'>Disclaimer:- This is a system generated email. Please do not reply to this email.\n"
				            + "</p>\n" +
				            "<p style='font-weight: bold;'>Team - Kite Cinemas</p> </body></html>";
	
				            

				    try {
				        helper.setText(emailBody, true);
				        helper.setTo(object.getEmail());
